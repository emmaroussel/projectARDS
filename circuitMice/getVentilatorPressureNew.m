function pressureSignal = getVentilatorPressureNew(time,ventilatorSettings)
% Function which computes ventilator pressure waveform based on given
% ventilator settings and time vector. "New" pressure waveform is herein
% applied (i.e., waveform with linear inspiratory phase and exponential
% expiratory phase).
%
% INPUTS:
...
%   tauInsp, tauExp). tauExp is unnecessary here.
% OUTPUT:
...
% Sep 2022
    ...
    for i = 1:length(time)
        ...
        if t < durationInspiration
            pressureSignal(i) = pmin + t*(pmax-pmin)/durationInspiration;
        else 
